package org.frictional.HandleGE;
...
public class ItemBuying {
    ...
    public void BuyItems(RequiredItem[] requiredItems) {
        ...
        if (Bank.isOpen()) {
            if (Interfaces.getComponent(664, 29) != null && Interfaces.getComponent(664, 29).interact("Close")) {
                Time.sleepUntil(() -> Interfaces.getComponent(664, 29) == null, Random.nextInt(2000, 3500));
            }
        ...
        if (Inventory.getCount(true, "Coins") > Main.keepMoney && Main.STARTMULER == false
                && Main.getLevelEvent() >= 55) {
            Main.updateScriptState(State.TRADEMULER);
        }
    ...
    public static RequiredItem[] getAllItemsToBuy(RequiredItem[] allRequiredItems) {
        ...
        for (RequiredItem requiredItem : allRequiredItems) {
            ...
            if(requiredItem.getName().endsWith(")")) {
                String t = requiredItem.getName().toString().split("\\(")[0];
                if (Inventory.getItems(x -> x.getName().contains(t)) != null
                        || Bank.getItems(x -> x.getName().contains(t)) != null){
                    Log.fine(requiredItem.getName());
    ...
    private void buySingleItem(RequiredItem item) {
        if (!GrandExchangeSetup.isOpen()) {
            if (GrandExchange.createOffer(RSGrandExchangeOffer.Type.BUY))
                Time.sleepUntil(GrandExchangeSetup::isOpen, Random.nextInt(4500, 8000));
            return;
        }
        ...
        if (currentSelectedItem == null || !currentSelectedItem.getName().equalsIgnoreCase(item.getName())) {
            if (GrandExchangeSetup.setItem(item.getName()))
                Time.sleepUntil(() -> GrandExchangeSetup.getItem() != null, Random.nextInt(4500, 8000));
            return;
        }
        //Set Quantity
        if (GrandExchangeSetup.getQuantity() != item.getAmountToBuy()) {
            if (GrandExchangeSetup.setQuantity(item.getAmountToBuy()))
                Time.sleepUntil(() -> GrandExchangeSetup.getQuantity() == item.getAmountToBuy(), Random.nextInt(4500, 8000));
            return;
        }
        //Set price
        if (item.getPrice() == -1) {
            ...
            if (GrandExchangeSetup.getPricePerItem() != item.getPrice()) {
                if (GrandExchangeSetup.setPrice(item.getPrice()))
                    Time.sleepUntil(() -> GrandExchangeSetup.getPricePerItem() == item.getPrice(), Random.nextInt(4500, 8000));
                return;
            }
        }
    ...
    private boolean cancelOffer(RSGrandExchangeOffer offer) {
        ...
        if (offer.abort())
            Time.sleepUntil(() -> GrandExchange.getOffers(x -> x.getProgress() == RSGrandExchangeOffer.Progress.FINISHED
                    && x.getItemName().equalsIgnoreCase(offer.getItemName())).length > 0, Random.nextInt(4500, 8000));
    ...
    private void collectAllOffers() {
        ...
        if (GrandExchange.collectAll())
            Time.sleepUntil(() -> Coins != Inventory.getCount("Coins"), Random.nextInt(4500, 8000));
    }
    ...
    private void withdrawCoinsFromBank() {
        ...
        if (Interfaces.getComponent(664, 29) != null && Interfaces.getComponent(664, 29).getComponent(0).interact("Close")) {
            Time.sleepUntil(() -> Interfaces.getComponent(664, 29) == null, Random.nextInt(2000, 3500));
        }
        ...
        if (Bank.getCount("Coins") == 0 && Inventory.contains("Coins")) {
