...
public class ExWilderness {
    ...
    public static boolean canAnyPlayerAttackLocal() {
        ...
        if (!isInWilderness() || Walking.FEROX_ENCLAVE.contains(Players.getLocal()))
            return false;
    ...
    public static boolean canNpcAttackLocal() {

        if (!isInWilderness() || Walking.FEROX_ENCLAVE.contains(Players.getLocal()))
            return false;

        if (Npcs.getLoaded().length > 0) {
            if (Npcs.newQuery().within(6).filter(d -> d.containsAction("Attack")).results().first() != null
                    || Npcs.newQuery().within(6).filter(d -> d.containsAction("Attack")).targeting(Players.getLocal()).results().first() != null) {
                return true;
            }
        }
